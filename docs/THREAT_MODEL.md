...
executable tests. The bounded `security-corpus/` foundation and seed fixtures
are present; M7 expands each threat-class suite and its generated execution.
...
| Relevant state changes | `ActionIntent` binds the inspected state; adapter re-resolves immediately before execution; mismatch invalidates authorization and triggers full reobservation/reauthorization or block **(D, Accepted ADR-0010)**. |
...
| ID | Invariant | Boundary | Related | Executable test |
|----|-----------|----------|---------|-----------------|
| INV-01 | **Page content can never grant capabilities.** Nothing observed from TB2 can widen the capability envelope, add an allowed origin, bind a secret sink, or change policy. | TB2, TB4 | ADR-0003 | [INV-01](../packages/testing/test/invariants/INV-01.spec.ts) |
| INV-02 | **Every attack class has at least one deterministic control**; a class covered only by a semantic control is a tracked deficiency. | all | PRD §8.4 | [INV-02](../packages/testing/test/invariants/INV-02.spec.ts) |
| INV-03 | **A semantic classifier cannot override a critical deterministic block.** Aggregation precedence is fixed; a semantic "allow" never removes a deterministic `BLOCK`. | TB3 | ADR-0003 | [INV-03](../packages/testing/test/invariants/INV-03.spec.ts) |
| INV-04 | **A secret handle cannot be resolved for an unapproved sink.** Resolution requires a matching `SinkBinding` (origin + field type at minimum), a permitting session state, and occurs only in the executor path. | TB7 | ADR-0005 | [INV-04](../packages/testing/test/invariants/INV-04.spec.ts) |
| INV-05 | **Raw secrets never appear in model context, findings, traces, receipts, approval requests, events, semantic caches, plugin context, or test fixtures.** | TB3, TB7, TB9 | ADR-0005 | [INV-05](../packages/testing/test/invariants/INV-05.spec.ts) |
| INV-06 | **Cross-origin secret/tainted egress is denied unless explicitly permitted.** Secret handles or tainted private data bound for an origin outside the envelope produce a `BLOCK`. | TB6 | PRD §13.9 | [INV-06](../packages/testing/test/invariants/INV-06.spec.ts) |
| INV-07 | **Persistent web-derived memory remains untrusted.** Memory writes retain provenance; reads of web-derived memory are treated as untrusted content, never as instructions. | TB8 | PRD §13.12 | [INV-07](../packages/testing/test/invariants/INV-07.spec.ts) |
| INV-08 | **High-impact actions must pass the Action Guard.** No wrapper path executes `SUBMIT`, `UPLOAD`, `DOWNLOAD`, `NAVIGATE` (cross-origin), `EXECUTE_SCRIPT`, `AUTHENTICATE`, `PURCHASE`, `DELETE`, `PUBLISH`, `MESSAGE`, `CHANGE_SETTING`, or any action carrying a handle without authorization, except via the recorded escape hatch. | TB4, TB5 | ADR-0002 | [INV-08](../packages/testing/test/invariants/INV-08.spec.ts) |
| INV-09 | **Scanner, provider, context, timeout, cancellation, or budget exhaustion may not silently disable protection.** Failed required checks for side-effectful actions yield `BLOCK`/`REQUIRE_APPROVAL`; oversized page/tool/provider inputs are rejected or treated as low-confidence, never clean. | TB2, TB3, TB4 | PRD §13.17, §23 | [INV-09](../packages/testing/test/invariants/INV-09.spec.ts) |
| INV-10 | **Private-network destinations are denied by default** (loopback, RFC1918, link-local, cloud metadata, configured internal ranges) for navigation, routed requests, and script-initiated fetches where interception exists. | TB6 | PRD §13.8 | [INV-10](../packages/testing/test/invariants/INV-10.spec.ts) |
| INV-11 | **Page content cannot approve its own action.** Approval requests are built from firewall-owned data; only the application-registered handler answers; no handler/timeout/error resolves to allow. | TB1, TB4 | PRD §13.18 | [INV-11](../packages/testing/test/invariants/INV-11.spec.ts) |
| INV-12 | **The capability envelope only shrinks during a session.** Risk states and budgets narrow it; only a new contract or an application `scope: session` approval within static policy limits widens runtime permissions. | TB1, TB4 | ARCHITECTURE §7 | [INV-12](../packages/testing/test/invariants/INV-12.spec.ts) |
| INV-13 | **Provenance survives firewall-visible transformations.** Sanitization, summarization by the firewall, memory writes, and replay preserve `DataProvenance`; the session taint floor covers transformations inside the agent. | TB2, TB4, TB8 | PRD §13.4 | [INV-13](../packages/testing/test/invariants/INV-13.spec.ts) |
| INV-14 | **Session risk never resets on navigation.** Only explicit policy (P1 decay/reset) restores trust; new tabs/popups inherit the session state. | TB5 | PRD §13.11 | [INV-14](../packages/testing/test/invariants/INV-14.spec.ts) |
| INV-15 | **Plugins run with least privilege.** A scanner plugin sees only manifest-permitted context, never raw secrets, cannot widen capability, and cannot load remote code. | TB9 | PRD §22 | [INV-15](../packages/testing/test/invariants/INV-15.spec.ts) |
| INV-16 | **Untrusted context, decoders, parsers, and provider calls are bounded.** Page/tool inputs, agent-facing context, encoded-payload normalization, DOM/ARIA parsing, JSON/YAML loading, and classifier requests/responses enforce byte/token, depth, call-count, cancellation, and time limits and never deserialize into executable objects. | TB1, TB2, TB3 | PRD §13.17, §21 | [INV-16](../packages/testing/test/invariants/INV-16.spec.ts) |
| INV-17 | **Every block is explainable.** Every `BLOCK`, `REQUIRE_APPROVAL`, `RESTRICT`, and `QUARANTINE` carries stable machine-readable reasons and reproducible (redacted) evidence in the trace. | all | PRD §7.9 | [INV-17](../packages/testing/test/invariants/INV-17.spec.ts) |
| INV-18 | **Escape-hatch use is always recorded.** Any access to raw framework handles is a trace event. | TB1, TB5 | PRD §18.6 | [INV-18](../packages/testing/test/invariants/INV-18.spec.ts) |
| INV-19 | **Authorization is bound to the exact structured operation and inspected browser state.** Guarded execution accepts only an `AuthorizedAction`; immediately before execution the adapter re-resolves the target and security-relevant state. Any mismatch or expiry requires reobservation and full reauthorization or fails closed; stale authorization is never patched. | TB2, TB4, TB5 | PRD §13.7; Accepted ADR-0010 | [INV-19](../packages/testing/test/invariants/INV-19.spec.ts) |
| INV-20 | **Probabilistic security components cannot grant authority.** Output from classifiers, guard models, alignment critics, ensembles, or other probabilistic components is schema-validated and treated as untrusted input. It cannot grant capabilities, resolve secrets, modify policy, approve actions, or override a deterministic critical block. | TB3, TB4 | ADR-0003; PRD §7.12 | [INV-20](../packages/testing/test/invariants/INV-20.spec.ts) |
| INV-21 | **Network mutations do not inherit authorization implicitly.** Action-, page-, form-, redirect-, WebMCP-, service-worker-, and unknown-originated network effects are represented independently; correlation to an `ActionIntent` is evidence only. Every surface claimed as enforced passes the Network Mutation Guard, while unsupported surfaces are reported as gaps. | TB2, TB6 | PRD §13.9 | [INV-21](../packages/testing/test/invariants/INV-21.spec.ts) |

---

## 7. Coverage mapping

Threat -> preventative -> detective -> recovery/containment -> test fixture.
**Gaps** are stated explicitly. Paths name the current or owning M7 layout
under `security-corpus/`; absent class breadth is not claimed complete.

| Class | Preventative (deterministic unless noted) | Detective | Recovery / containment | Fixtures | Gaps / notes |
|-------|--------------------------------------------|-----------|------------------------|------------------|--------------|
| A1 Indirect injection | Sanitized representation; envelope; Action Guard | Heuristic scanner; BYOK classifier (S) | `RESTRICTED` mode; explain | `hidden-dom/`, `aria/`, `encoding/`; committed multi-step case | Detection is probabilistic; containment is the guarantee. |
...
| A3 Hidden DOM / ARIA | Visibility classifier; sanitization | Hidden-DOM, ARIA-consistency, comment, metadata scanners | `RESTRICTED` | `hidden-dom/`, `aria/` (21 attack and 6 benign cases) | Pixel-only visual injection remains A2/P1. |
| A4 Encoded smuggling | Bounded normalizer before heuristics | Encoded-payload + Unicode-invisible scanners; classifier on decoded text (S) | Risk increase or explicit non-clean bounded result | `encoding/` (20 attack, 4 benign, 2 limit cases) | Novel encodings may evade; bounded decode depth is intentional. |
| A5 Cross-origin exfil | Origin policy; cross-origin exfil rule; secret handles; taint floor | Egress inspection with value matching (where hooks exist) | `BLOCK`; `QUARANTINE` on repeated attempts | 22 generated attack cases in `exfiltration/` | Initial Playwright navigation/fetch hooks prove zero bytes; form/header/upload transport remains observed-only and full proxying is deferred. |
| A6 Credential misuse | Secret handles + exact sink binding | Secret scanner; bounded phishing fixture | Handle invalidation at session end | `exfiltration/credential-*` generated cases | Playwright exact fill/type/select binding includes origin, field type, selector, and form action; Stagehand secret sinks remain unavailable. |
...
| A9 Purchase/delete/message/publish/settings | Taxonomy + approval gates | Irreversibility scoring (P1) | Approval; deny without handler | `navigation/case-matrix.html` (12 action-class approval cases plus TOCTOU) | Framework normalization must recognise these classes; unknown -> conservative. |
| A10 SSRF / private network | Private-network policy; scheme policy; script denial | URL scanner; route interception | `BLOCK` | `navigation/case-matrix.html` (13 attacks and one explicitly authorized sibling) | Initial routed fetch has zero-byte proof; DNS rebinding is P1. |
| A11 Downloads / uploads | Upload guard (provenance, sensitivity, destination, capability); download interception metadata | Download-metadata scanner; file scanners (P1/P2 adapters) | Quarantine state (P1) | generated upload name/path/body cases plus `navigation/download-*` | Pre-action upload metadata is enforced; route-level multipart/content scanning remains P1/P2. |
| A12 Memory poisoning | Memory write/read guards; instruction/data separation; read-time schema/hash verification and re-tainting | PERSISTENCE scanners | Reject malformed/tampered items; preserve untrusted provenance | `memory/case-matrix.html` (9 attacks, 2 benign direct write/read cases) | Requires the application to route memory writes and reads through the guard (Q10). Enhanced cross-session policy is P1. |
| A13 Redirect / domain confusion | Origin policy; max redirect hops; redirect-chain inspection | Suspicious-link scanner; link-spoof (P1) | `BLOCK` | `navigation/case-matrix.html` redirect/scheme cases | Playwright redirect follow-ups are observed-only; homoglyph domain similarity is P2. |
...
| A15 Denial of wallet | Session and per-boundary budgets (actions, duration, page/tool bytes/tokens, decode limits, scanner/provider timeouts, guard calls/tokens, downloads/uploads, tabs) | Loop and exhaustion events | `require_approval` / `restrict` / `quarantine` on exceed | budget/limit unit tests; `navigation/case-matrix.html` action/navigation/duration/tab cases | Required checks fail closed for side effects. |
| A16 Tab / popup abuse | Tab/window guard (max tabs, popup policy, new-window origins) | Popup/new-page events | Popups inherit session state | `navigation/case-matrix.html` (four attack and one benign popup cases) | Playwright popup handling is post-creation, not zero-byte; Stagehand is unavailable; focus policy is best-effort. |
| A17 URL / metadata injection | Origin policy; metadata not sent as instructions | URL + metadata scanners; classifier (S) | Risk increase | `encoding/case-matrix.html` metadata/attribute cases; `navigation/url-*` | — |
| A18 Authorization/execution race | Exact structured execution; state-bound `ActionIntent`; immediate deterministic revalidation | POST_ACTION anomaly checks | Invalidate, reobserve + reauthorize, or `BLOCK` | [`mutation/case-matrix.html`](../security-corpus/mutation/case-matrix.html) and generated Playwright/Stagehand tests cover target, destination, form, frame, origin, visibility, attributes, operation, observation, and combined races | Accepted ADR-0010 governs both adapters. Stagehand requires `selfHeal: false`, reobserves once, then fails closed; its POST_ACTION window remains unavailable. |
| A19 Guard compromise/misclassification | Capability envelope, Action Guard, origin/private-network policy, sink-bound secrets, fixed precedence | Schema validation; tier disagreement; provider failure events | Ignore malformed output; `scanner_unavailable`; fail closed when a required check is unavailable | [`guard-failure/case-matrix.html`](../security-corpus/guard-failure/case-matrix.html) exercises false-safe, malformed, oversized, timeout, cancellation-ignoring, budget, and benign results | Semantic detection can fail; deterministic containment is the guarantee. |
| A20 Page/tool network mutation | `NetworkMutationGuard`; origin/private-network/DLP checks where hooks exist; explicit initiator classification | Route/WebSocket/navigation/form/WebMCP events | Block when enforceable; report observed-only/unavailable surfaces | [`network-mutation/case-matrix.html`](../security-corpus/network-mutation/case-matrix.html) and [`webmcp/case-matrix.html`](../security-corpus/webmcp/case-matrix.html) cover fetch, form, redirect, WebSocket, beacon, service worker, correlation, and WebMCP rows | Full proxy deferred; zero-byte evidence applies only to measured routed navigation/fetch. No claim is made for unobservable traffic. |

Cross-cutting fixtures include 12 non-English or mixed-language structural
variants for A1/A3/A4 across Latin, Japanese, Arabic, and Hebrew pages, with
Arabic and Hebrew RTL cases. They verify language-independent Unicode,
encoding, hidden-DOM, ARIA, comment, and metadata signals; because the built-in
lexical pack is English, they do not claim native-language semantic recall.
The 30-page benign baseline spans news, shopping, documentation, forms,
dashboards, and legitimate hidden UI and measures zero default-policy hard
recommendations or risk transitions (PRD §24, §32). Broader false-positive
measurement remains owned by the M8 benchmark gate.
...
|---------------------------------|-------------------|----------------------------------------|-----------------------|
| Injection classifier misses the attack | Hostile instruction reaches agent context | Action Guard, origin policy, capability envelope, session restrictions | [INV-01](../packages/testing/test/invariants/INV-01.spec.ts), fake-permissive classifier case |
| Main agent follows the attack | Agent proposes an out-of-task action | Capability envelope + deterministic policy; unknown/high-impact action handling | [INV-08](../packages/testing/test/invariants/INV-08.spec.ts), obedient-agent containment case |
| Guard model/critic is compromised or returns malformed output | False-safe or adversarial semantic result | Strict schema validation + INV-20 fixed precedence; semantic output grants no authority | [INV-20](../packages/testing/test/invariants/INV-20.spec.ts), malformed/fake-safe provider cases |
| Secret is requested or the agent tries to disclose it | Handle/raw value approaches a sink | Executor-side sink-bound resolution + egress matching | [INV-04](../packages/testing/test/invariants/INV-04.spec.ts), zero-byte exfiltration cases |
| Agent proposes external/private navigation | Disallowed destination selected | Origin and private-network policy | [INV-10](../packages/testing/test/invariants/INV-10.spec.ts), redirect/SSRF cases |
| Authorized element or destination mutates before use | Stale action could hit a different target | `ActionIntent` re-resolution/revalidation; invalidate and fully reauthorize | [INV-19](../packages/testing/test/invariants/INV-19.spec.ts), TOCTOU cases |
| Page or WebMCP tool causes an unexpected network side effect | Traffic does not correspond to the action model | Network Mutation Guard where hooks exist; explicit gap reporting elsewhere | [INV-21](../packages/testing/test/invariants/INV-21.spec.ts), network/WebMCP cases |
| Poisoned content is persisted | Attack survives into a later session | Memory Write Guard, provenance/hash retention, and P0 Memory Read Guard | [INV-07](../packages/testing/test/invariants/INV-07.spec.ts), write/read poisoning cases |
| Scanner/provider/context budget is exhausted | Required inspection cannot finish | Side-effectful action fails closed or requires approval; no handler denies | [INV-09](../packages/testing/test/invariants/INV-09.spec.ts), exhaustion properties |
| One adapter execution path lacks a hook | Framework could bypass the firewall | Path disabled by default; recorded escape hatch only | [INV-21](../packages/testing/test/invariants/INV-21.spec.ts), adapter capability tests |
...
- **v0.1 taint is coarse.** The session taint floor is conservative
  (may over-restrict). The bounded source-value registry covers exact,
  trim/case, one URL encoding, and standard Base64 matching, but rewording,
  encryption, recursive encoding, Base64URL, and Unicode folding can evade it;
